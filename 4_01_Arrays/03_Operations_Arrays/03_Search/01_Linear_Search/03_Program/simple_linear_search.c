...
Description:
The program uses linear search to check whether a target element is present in a given array. If found, the program prints the index of the element; otherwise, it prints that the element is not present.
...
Output:
The program outputs the result of the linear search, either displaying the index where the element is found or a message indicating that the element is not in the array.
...
int linearSearch(int farr[], int fn, int ftarget) {
    ...
    for (fi = 0; fi < fn; fi++) {
        if (farr[fi] == ftarget) {
            return fi; // Found the target at index fi
        }
    }
